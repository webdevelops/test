import {
  FETCH_PHONES_REQUEST,
  FETCH_PHONES_SUCCESS,
  FETCH_PHONES_FEILURE,
  LOAD_MORE_PHONES_REQUEST,
  LOAD_MORE_PHONES_SUCCESS,
  LOAD_MORE_PHONES_FEILURE,
  FETCH_PHONE_BY_ID_REQUEST,
  FETCH_PHONE_BY_ID_SUCCESS,
  FETCH_PHONE_BY_ID_FEILURE,
  ADD_PHONE_TO_BASKET,
  SEARCH_PHONE,
  FETCH_CATEGORIES_REQUEST,
  FETCH_CATEGORIES_SUCCESS,
  FETCH_CATEGORIES_FEILURE,
  CLEAR_BASKET,
  CHECKOUT_BASKET,
  REMOVE_PHONE_FROM_BASKET,
  AUTH_REQUEST,
  AUTH_SUCCESS,
  AUTH_FEILURE,
  AUTH_LOGOUT
} from 'internetShop/actions/actionTypes'

import {
  fetchPhonesApi,
  loadMorePhonesApi,
  fetchPhoneByIdApi,
  fetchCategoriesApi,
  authApi
// } from 'internetShop/api'
} from 'internetShop/api/mockAPI'

// import handleError from 'Mui-3/auth/SignIn'

export const fetchPhones = () => async dispatch => {
  dispatch({
    type: FETCH_PHONES_REQUEST
  })

  try {
    const phones = await fetchPhonesApi()
    dispatch({
      type: FETCH_PHONES_SUCCESS,
      payload: phones
    })

  } catch (err) {
    dispatch({
      type: FETCH_PHONES_FEILURE,
      payload: err,
      error: true
    })
  }
}

export const loadMorePhones = () => async dispatch => {
  dispatch({
    type: LOAD_MORE_PHONES_REQUEST
  })

  try {
    const phones = await loadMorePhonesApi()
    dispatch({
      type: LOAD_MORE_PHONES_SUCCESS,
      payload: phones
    })

  } catch (err) {
    dispatch({
      type: LOAD_MORE_PHONES_FEILURE,
      payload: err,
      error: true
    })
  }
}

export const fetchPhoneById = id => async dispatch => {
  dispatch({
    type: FETCH_PHONE_BY_ID_REQUEST
  })

  try {
    const phone = await fetchPhoneByIdApi(id)
    dispatch({
      type: FETCH_PHONE_BY_ID_SUCCESS,
      payload: phone
    })

  } catch (err) {
    dispatch({
      type: FETCH_PHONE_BY_ID_FEILURE,
      payload: err,
      error: true
    })
  }
}

export const addPhoneToBasket = id => dispatch => {
  dispatch({
    type: ADD_PHONE_TO_BASKET,
    payload: id
  })
}

export const searchPhone = value => dispatch => {
  dispatch({
    type: SEARCH_PHONE,
    payload: value
  })
}

export const fetchCategories = () => async dispatch => {
  dispatch({
    type: FETCH_CATEGORIES_REQUEST
  })

  try {
    const categories = await fetchCategoriesApi()
    dispatch({
      type: FETCH_CATEGORIES_SUCCESS,
      payload: categories
    })

  } catch (err) {
    dispatch({
      type: FETCH_CATEGORIES_FEILURE,
      payload: err,
      error: true
    })
  }
}

export const clearBasket = () => dispatch => {
  dispatch({
    type: CLEAR_BASKET
  })
}
export const checkoutBasket = phones => dispatch => {
  dispatch({
    type: CHECKOUT_BASKET,
    payload: phones
  })
}

export const removePhoneFromBasket = id => dispatch => {
  dispatch({
    type: REMOVE_PHONE_FROM_BASKET,
    payload: id
  })
}

export const auth = (email, password, isLogin) => async dispatch => {
  let response = {};

  dispatch({
    type: AUTH_REQUEST
  });

  try {
    response = await authApi(email, password, isLogin)
    if (response.error) {
      alert(response.error.message)
      throw new Error(response.error.message)
    }
    dispatch({
      type: AUTH_SUCCESS,
      payload: response.idToken
    });

    // const expirationToken = await autoLogout(response.expiresIn * 1);  // -------- variant 1
    // dispatch(expirationToken);

    dispatch(autoLogout(response.expiresIn * 10));

  } catch (err) {
    dispatch({
      type: AUTH_FEILURE,
      payload: response.error.message,
      error: true
    })
  }
};

export const logout = () => {
  localStorage.removeItem('token')
  localStorage.removeItem('localId')
  localStorage.removeItem('expirationDate')

  return {
    type: AUTH_LOGOUT
  };
};

// export const autoLogout = delay => {    // -------- variant 1
//   return new Promise(resolve => {
//     const timerId = setTimeout(() => {
//       resolve(logout());

//       clearTimeout(timerId);
//     }, delay)
//   });
// };

export const autoLogout = delay => {
  return dispatch => {
    const timerId = setTimeout(() => {
      dispatch(logout());

      clearTimeout(timerId);
    }, delay);
  }
};

export const autoLogin = () => async dispatch => {
  const token = localStorage.getItem('token');
  console.log("TCL: token", token)

  if (!token) {
    dispatch(logout());

  } else {
    const expirationDate = new Date(localStorage.getItem('expirationDate'));

    if (expirationDate <= new Date()) {
      dispatch(logout());

    } else {
      dispatch({ type: AUTH_SUCCESS, payload: token });
      
      const delay = expirationDate.getTime() - new Date().getTime();
      dispatch(autoLogout(delay));

      // const expirationToken = await autoLogout(delay);    // -------- variant 1
      // dispatch(expirationToken);     
    }
  }
};