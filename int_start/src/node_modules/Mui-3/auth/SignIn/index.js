...
const SignIn = ({ auth, error }) => {
  ...
  const handleError = Transition => {
    console.log("TCL: SignIn -> true", true)
  ...
  const handleClickAuth = async () => {
    const responsel = await auth(
      formControls.email.value,
      formControls.password.value,
      true
    )
    console.log("TCL: handleClickAuth -> response", responsel)
    // const json = await response.json()
    // console.log("TCL: handleClickAuth -> json", json)
    if (responsel) {
      // console.log("TCL: SignIn -> error !!!!!!!", error)
      handleError(TransitionLeft)
    }
    // setTimeout(() => {
    //   console.log("TCL: handleClickAuth -> response-2", responsel)
    // }, 1000)
  }
